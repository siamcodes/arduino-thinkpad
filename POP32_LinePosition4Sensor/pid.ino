...
void stdPID(int baseSpeed, float  Kp, float Kd, int maxsp, int minsp) {
  ...
  int Error = Pos - 2000;  //ตัวละ 1000x4 sensor=4000 หาร 2=2000 (ค่ากลาง) หรือกำหนดค่าให้หุ่นอยู่ตรงกลาง
...
void FF(int spd, float kp , float kd, char select, int dl, int maxsp, int minsp) {
  while (1) {
    readCalibrate();
    //if ((F[0] <= REF[0] && F[1] <= REF[1] && F[2] <= REF[2] && F[3] <= REF[3] )) //เขียวมาก ขาวน้อย
      if ((F[0] >= REF[0] && F[1] >= REF[1] && F[2] >= REF[2] && F[3] >= REF[3] ))  //เขียวน้อย ขาวมาก
    {
      break;
    }
...
void FFR(int spd, float kp , float kd, char select, int dl, int maxsp, int minsp) {
  while (1) {
    readCalibrate();
    if (((F[3] <= REF[3] && F[2] <= REF[2]) && (F[0] >= REF[0] && F[1] >= REF[1] )))
    {
      break;
    }
    stdPID(spd, kp, kd , maxsp, minsp);
