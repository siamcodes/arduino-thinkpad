...
void stdPID(int baseSpeed, float  Kp, float Kd, int maxsp, int minsp) {
  ...
  int Error = Pos - 3500;  //ตัวละ 1000x7 sensor=7000 หาร 2=3500 (ค่ากลาง)
...
void SpinL() {
  ...
  Motor(-40, 40); delay(500); MotorStop();
  while (1) {
    readCalibrate();
    if (F[0] <= REF[0]|| F[1] <= REF[1]) {  //ค่าเดิม 1
      MotorStop();
...
void SpinR() {
  ...
  while (1) {
    readCalibrate();
    if (F[5] <= REF[5] || F[6] <= REF[6]) {  //ค่าเดิม3
      MotorStop();
      Motor(40, -40); delay(10);
      break;
    }
